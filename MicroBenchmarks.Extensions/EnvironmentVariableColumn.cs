...
namespace MicroBenchmarks.Extensions
{
	...
	public class EnvironmentVariableColumn : IColumn
	{
		...
		public EnvironmentVariableColumn(string columnName, string environmentVariableKey, string defaultValue = "Not set")
		{
			...
			{
				cellValue = cellValue ?? Environment.GetEnvironmentVariable(environmentVariableKey, EnvironmentVariableTarget.User);
				cellValue = cellValue ?? Environment.GetEnvironmentVariable(environmentVariableKey, EnvironmentVariableTarget.Machine);
			}

			cellValue = cellValue ?? defaultValue;
